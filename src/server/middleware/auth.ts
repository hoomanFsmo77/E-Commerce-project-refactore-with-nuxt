...
export default defineEventHandler(async ev=>{
    ...
    if(url==='/api/navbar' || url==='/api/collection/all' || url==='/api/popular' || url==='/api/popular' || url==='/api/search' || url==='/api/recent' || url==='/api/instagram' || url?.includes('/api/collection') || url?.includes('/api/product') || url?.includes('/api/pages') || url?.includes('/api/policy')){
